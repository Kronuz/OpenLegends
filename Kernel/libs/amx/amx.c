...
/* Added by GD and Kronuz for the Open Zelda Project */
  amxClone->firstrun = amxSource->firstrun;
  strcpy(amxClone->szFileName, amxSource->szFileName);
  amxClone->flags = amxSource->flags; // did the creators of Small forgot this?!?
...
//  NEXT(cip);
  ...
  op_line:
    ...
    if(amx->flags&AMX_FLAG_ABORT) { /* (Kronuz) */
		ABORT(amx,num);
	} else if (debug) {
      amx->frm=frm;
      amx->stk=stk;
...
int AMXAPI amx_Exec(AMX *amx, cell *retval, int index, int numparams, ...)
{
  ...
  amx->flags &= ~AMX_FLAG_ABORT; /* (Kronuz) */
...
{
  u_char FAR *data; /* (Kronuz) */
  AMX_HEADER *hdr=(AMX_HEADER *)amx->base;

  if (amx_addr>=amx->hea && amx_addr<amx->stk || amx_addr<0 || amx_addr>=amx->stp)
    return AMX_ERR_MEMACCESS;

  /* Modified by Kronuz. August 12, 2003, the address was always got from the 
     base address, even when a data segment (amx->data) existed */
  data=(amx->data!=NULL) ? amx->data : amx->base+(int)hdr->dat;
