...
vector<const string*> ASFormatter::preVariableHeaders;
...
//    nonParenHeaders.push_back(&AS_TEMPLATE);
	...
	preVariableHeaders.push_back(&AS_NEW); // added by Kronuz
...
string ASFormatter::nextLine()
{
    ...
    while (!isLineReady)
    {
        ...
        if (!isInTemplate)
        {
			...
			else if ( (newHeader = findHeader(preVariableHeaders)) != NULL)
            {
                foundPreVariableHeader = true;
                appendSequence(*newHeader);
                goForward(newHeader->length() - 1);
        ...
        if (currentChar == ':'
				...
				&& !foundPreVariableHeader // not in a variable definition (e.g. new float : name
